...
def add_box(matrix, *args) # 6 args: x, y, z, width, height, depth (x, y, z represents top-left-front corner)
    ...
    add_triangle(matrix, x, y, z, x, ny, z, nx, y, z) # front face
    add_triangle(matrix, nx, y, z, x, ny, z, nx, ny, z)
    add_triangle(matrix, nx, y, z, nx, ny, z, nx, y, nz) # right face
    add_triangle(matrix, nx, y, nz, nx, ny, z, nx, ny, nz)
    add_triangle(matrix, nx, y, z, nx, y, nz, nx, ny, nz) # back face
    add_triangle(matrix, x, y, nz, nx, ny, nz, x, ny, nz)
    add_triangle(matrix, x, y, z, x, y, nz, x, ny, nz) # left face
    add_triangle(matrix, x, y, z, x, ny, nz, x, ny, z)
    add_triangle(matrix, x, y, nz, x, y, z, nx, y, nz) # top face
    add_triangle(matrix, nx, y, nz, x, y, z, nx, y, z)
    add_triangle(matrix, x, ny, z, x, ny, nz, nx, ny, z) # bottom face
    add_triangle(matrix, nx, ny, z, x, ny, nz, nx, ny, nz)
end
